...
# En Render, configure un Persistent Disk con mount path /var/data y la
# variable de entorno AULA_DATA_DIR=/var/data. De esta forma la base SQLite
# y los archivos subidos sobreviven a reinicios y nuevos despliegues.
def _resolve_data_dir() -> Path:
    configured = os.getenv("AULA_DATA_DIR", "").strip()
    if configured:
        return Path(configured).expanduser().resolve()

    # Valor seguro por defecto para Render. IMPORTANTE: /var/data será
    # persistente únicamente si existe un Persistent Disk montado ahí.
    if os.getenv("RENDER"):
...
def db():
    ...
    # Mejora de integridad y tolerancia a accesos concurrentes.
...
def backup_database():
    """Crea una copia consistente de SQLite dentro del almacenamiento persistente."""
    if not DB.exists():
        return None
    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    target = BACKUPS / f"aula_{stamp}.db"
    ...
    try:
        ...
        dst.close()
        src.close()
    # Conservar solo los 10 respaldos más recientes.
    backups = sorted(BACKUPS.glob("aula_*.db"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def startup():
    ...
    try:
        ...
        print(f"[AULA] No se pudo crear respaldo de inicio: {exc}")
    print(f"[AULA] DATA_DIR={DATA_DIR}")
    if os.getenv("RENDER") and not os.getenv("AULA_DATA_DIR"):
        print("[AULA] ADVERTENCIA: configure AULA_DATA_DIR=/var/data y monte un Persistent Disk en /var/data para no perder datos.")
...
def create_backup(request: Request):
    ...
    return {"ok": True, "backup": target.name if target else None}
...
def health():
    ...
    return {
        ...
        "persistent_dir_configured": bool(os.getenv("AULA_DATA_DIR")),
    }
