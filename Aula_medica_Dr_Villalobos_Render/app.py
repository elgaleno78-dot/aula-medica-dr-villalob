...
def validate_course_access(course_id:int, token:str):
    ...
    con=db()
    row=con.execute("""
      SELECT s.id,s.full_name,s.email,s.course_id,s.access_started_at,s.access_expires_at
      FROM students s
...
def course_access(course_id:int, access_token:str=""):
    ...
    lessons=[dict(r) for r in con.execute("""
      SELECT id,course_id,title,kind,filename,notes,ord,created_at
    ...
    return {
      ...
      "expires_at":info["access_expires_at"],
...
def lesson_file(lesson_id:int, access_token:str=""):
    con=db()
    row=con.execute("SELECT id,course_id,filename,kind,title FROM lessons WHERE id=?",(lesson_id,)).fetchone()
    con.close()
...
async def update_course_access_settings(course_id:int, request:Request):
    ...
    if hours<1 or hours>8760:
        raise HTTPException(400,"La duración debe ser entre 1 y 8760 horas")
    con=db()
...
async def register_student(
    ...
    con=db(); cur=con.cursor()
    course=cur.execute(
      "SELECT id,title,COALESCE(access_hours,24) access_hours FROM courses WHERE id=?",
      (course_id,)
    ).fetchone()
    if not course:
        con.close()
    ...
    if email:
        dup=cur.execute(
          "SELECT id,access_expires_at FROM students WHERE lower(email)=? AND course_id=? ORDER BY id DESC LIMIT 1",
          (email,course_id)
        ).fetchone()
        if dup:
            con.close()
            raise HTTPException(409,"Este correo ya fue inscrito en el curso")
    ...
    cur.execute("""
      INSERT INTO students(
        full_name,age,institution,position,email,course_id,
        access_token,access_started_at,access_expires_at
      ) VALUES(?,?,?,?,?,?,?,?,?)
    """,(full_name,age,institution.strip(),position.strip(),email,course_id,
         token,started.isoformat(),expires.isoformat()))
    sid=cur.lastrowid
    cur.execute(
      "INSERT INTO activities(message) VALUES(?)",
      (f"Nuevo alumno inscrito: {full_name} · acceso {hours} h",)
    )
    con.commit(); con.close()
    return {
      "ok":True,
