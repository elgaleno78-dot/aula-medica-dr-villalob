...
def init_db():
    ...
    if "access_hours" not in course_cols: cur.execute("ALTER TABLE courses ADD COLUMN access_hours INTEGER DEFAULT 24")
    student_cols={r["name"] for r in cur.execute("PRAGMA table_info(students)").fetchall()}
    if "access_token" not in student_cols: cur.execute("ALTER TABLE students ADD COLUMN access_token TEXT DEFAULT ''")
    if "access_started_at" not in student_cols: cur.execute("ALTER TABLE students ADD COLUMN access_started_at TEXT")
    if "access_expires_at" not in student_cols: cur.execute("ALTER TABLE students ADD COLUMN access_expires_at TEXT")
...
def courses():
    ...
    rows=[dict(r) for r in con.execute("""
      ...
      FROM courses c WHERE c.published=1 ORDER BY c.id
    """).fetchall()]
    for r in rows: r["lessons"]=[]
    con.close()
    return rows
...
def validate_course_access(course_id:int, token:str):
    if not token: raise HTTPException(401,"Inscripción requerida")
    con=db()
    row=con.execute("""
      SELECT s.full_name,s.access_expires_at FROM students s
      WHERE s.course_id=? AND s.access_token=? ORDER BY s.id DESC LIMIT 1
    """,(course_id,token)).fetchone()
    con.close()
    if not row: raise HTTPException(401,"Acceso no válido")
    try:
    ...
    if remaining<=0: raise HTTPException(403,"El periodo de acceso ha terminado")
...
def course_access(course_id:int, access_token:str=""):
    ...
    lessons=[dict(r) for r in con.execute("""
      SELECT id,course_id,title,kind,notes,ord,created_at
      FROM lessons WHERE course_id=? ORDER BY ord,id
    """,(course_id,)).fetchall()]
    con.close()
    return {"course":dict(c),"lessons":lessons,"student":info["full_name"],
            "expires_at":info["access_expires_at"],"remaining_seconds":int(remaining)}

@app.get("/api/lesson-file/{lesson_id}")
def lesson_file(lesson_id:int, access_token:str=""):
    con=db()
    row=con.execute("SELECT id,course_id,filename,kind FROM lessons WHERE id=?",(lesson_id,)).fetchone()
    con.close()
    if not row or not row["filename"]: raise HTTPException(404,"Archivo no encontrado")
    validate_course_access(row["course_id"],access_token)
    p=UPLOADS/row["filename"]
    if not p.exists(): raise HTTPException(404,"Archivo no disponible")
...
async def update_course_access_settings(course_id:int, request:Request):
    ...
    if hours<1 or hours>8760: raise HTTPException(400,"Duración inválida")
    con=db(); con.execute("UPDATE courses SET access_hours=? WHERE id=?",(hours,course_id)); con.commit(); con.close()
...
async def register_student(
    full_name:str=Form(...), age:int|None=Form(None), institution:str=Form(""),
    position:str=Form(""), email:str=Form(""), course_id:int=Form(...)
):
    full_name=full_name.strip(); email=email.strip().lower()
    if len(full_name)<3: raise HTTPException(400,"Nombre inválido")
    con=db(); cur=con.cursor()
    ...
    if email:
        ...
        if dup:
            con.close(); raise HTTPException(409,"Este correo ya fue inscrito en el curso")
    hours=max(1,int(course["access_hours"] or 24))
    started=utcnow(); expires=started+datetime.timedelta(hours=hours)
    ...
    cur.execute("""INSERT INTO students(full_name,age,institution,position,email,course_id,access_token,access_started_at,access_expires_at)
                   VALUES(?,?,?,?,?,?,?,?,?)""",
                (full_name,age,institution.strip(),position.strip(),email,course_id,token,started.isoformat(),expires.isoformat()))
    sid=cur.lastrowid
    cur.execute("INSERT INTO activities(message) VALUES(?)",(f"Nuevo alumno inscrito: {full_name} · acceso {hours} h",))
    con.commit(); con.close()
    return {"ok":True,"id":sid,"course_id":course_id,"access_token":token,
            "access_hours":hours,"started_at":started.isoformat(),"expires_at":expires.isoformat()}
...
def list_students(request:Request):
    ...
    con=db()
    rows=[dict(r) for r in con.execute("""
      SELECT s.id,s.full_name,s.age,s.institution,s.position,s.email,s.course_id,s.access_started_at,s.access_expires_at,s.created_at,c.title AS course_title
      FROM students s
